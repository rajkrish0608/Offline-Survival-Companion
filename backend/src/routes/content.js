...
router.get('/packs', async (req, res) => {
    try {
        ...
        res.json({ message: 'Content packs retrieved successfully', packs });
    } catch (err) {
...
router.get('/packs/:packId', async (req, res) => {
    try {
        ...
        if (!pack) {
            return res.status(404).json({ error: 'Not Found', message: 'Pack not found' });
        }

        res.json({ message: 'Pack details retrieved successfully', pack });
    } catch (err) {
        res.status(500).json({ error: 'Internal Server Error', message: err.message });
    }
});
...
router.get('/first-aid', async (req, res) => {
    try {
        ...
        let query = 'SELECT * FROM first_aid_articles WHERE TRUE';
        const params = [];
        let paramIndex = 1;
        ...
        const articles = await dbAll(query + ' ORDER BY category ASC', params);

        res.json({ message: 'First aid articles retrieved successfully', articles });
    } catch (err) {
        res.status(500).json({ error: 'Internal Server Error', message: err.message });
    }
});
...
router.get('/procedures', async (req, res) => {
    try {
        const procedures = await dbAll(
            'SELECT * FROM emergency_procedures ORDER BY category ASC'
        );
        res.json({ message: 'Procedures retrieved successfully', procedures });
    } catch (err) {
        res.status(500).json({ error: 'Internal Server Error', message: err.message });
    }
});
...
router.get('/poi-regions', async (req, res) => {
    try {
        ...
        `);
        res.json({ message: 'POI regions retrieved successfully', regions });
    } catch (err) {
        res.status(500).json({ error: 'Internal Server Error', message: err.message });
    }
});
